"""
К решению задания 7.3 добавлены assert-ы согласно пунктам 5.1-5.7.
"""
...
class RoboPart:
    ...
    def __robopart_invar(self) -> None:
        assert len(self.__category) > 0, "Part has to have a category."
        assert len(self.__name) > 0, "Part has to have a name."
        assert self.__slot_size > 0, "Part has to have non-zero slot size."
        assert self.__integrity >= 0, "Part integrity can't be negative."
        assert self.__coverage >= 0, "Part coverage can't be negative."
    ...
    def coverage(self) -> int:
        assert self.__coverage >= 0, "Coverage can not be negative."
        return self.__coverage

    @invariant(__robopart_invar)
    def part_take_damage(self, part_dmg: int) -> None:
        assert part_dmg >= 0, "Taken damage can't be negative."
        # часть получает урон
    ...
    def part_repair(self, repair_dmg: int) -> None:
        assert repair_dmg >= 0, "Repaired damage can't be negative."
...
class Robot:
    ...
    def __init__(self, robot_name: str, robot_bhv: str,
        ...
        assert len(self.__name) > 0, "Robot has to have a name."
        assert len(self.__behavior) > 0, "Robot has to have a behavior."
        assert self.__core_integrity >= 0, "Core integrity can't be negative."
        assert self.__core_coverage > 0, "Core coverage has to be positive."
        assert self.__free_slots >= 0, "Number of free slots can't be negative."
    ...
    def attach_part(self, attached_part: RoboPart) -> None:
        assert isinstance(attached_part, RoboPart), "Only parts can be attached."
    ...
    def robot_take_damage(self, robot_damage: int) -> None:
        assert robot_damage >= 0, "Damage taken can't be negative."
        ...
        assert total_coverage > 0, "At least something must get hit."
        ...
        for i in range(part_count):
            ...
            if random_hit <= 0:
                ...
                return
        assert random_hit > 0 and random_hit <= self.__core_coverage, (
            "Core should be hit only if no other parts got hit.")
    ...
    def move(self, current_position: GridCell,
        ...
        assert isinstance(current_position, GridCell), "Current position must be a GridCell."
        assert isinstance(intended_position, GridCell), "Intended position must be a GridCell."
...
class GridCell:
    ...
    def __init__(self) -> None:
        ...
        assert len(self.__texture) == 1, "Texture has to be one-character long."
    ...
    def __update_texture(self) -> None:
        ...
        self.__texture = self.__robot.name[0]
        assert len(self.__texture) == 1, "Texture has to be one-character long."
    ...
    @property
    def wall(self) -> bool:
        assert isinstance(self.__wall, bool), "'Wall' property has to have a bool type."
        return self.__wall

    @wall.setter
    def wall(self, flag: bool) -> None:
        assert isinstance(flag, bool), "'Wall' property has to have a bool type."
        self.__wall = flag
    ...
    def robot_arrives(self, robot: Robot) -> None:
        assert isinstance(robot, Robot), "Only robots can move."
        self.__robot = robot
        self.__update_texture()
